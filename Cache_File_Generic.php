	...
	function set( $key, $var, $expire = 0, $group = '' ) {
		...
		if ( Util_Environment::is_apache() && isset( $var['headers'] ) ) {
			...
			if ( !empty($rules) ) {
				...
				$base_cache_path = W3TC_CACHE_PAGE_ENHANCED_DIR . DIRECTORY_SEPARATOR . Util_Environment::host();

				if( $uri_cache_path != $base_cache_path ){
				    @file_put_contents($uri_cache_path . DIRECTORY_SEPARATOR . '.htaccess', $rules);
				}
			}
		}
